...
public final class ChristmasHoliday: Holiday {
    ...
    public func isDuring(_ date: Date, calendar: Calendar) -> Bool {
        ...
        let start = calendar.date(from: .init(year: year, month: 12, day: 24))!
        let end = calendar.date(from: .init(year: year, month: 12, day: 26))!
        return date >= start && date <= end
    }

    public func nextOccurrence(after date: Date, calendar: Calendar) -> Date? {
        let year = calendar.component(.year, from: date)
        let thisYear = calendar.date(from: .init(year: year, month: 12, day: 24))!
